****************************************************************************
*                                                                          *
*  Readme-NitroSDK-3_1-patch-plus4-060807.txt                              *
*                                                                          *
...
This patch corrects problems found in NITRO-SDK after the 3.1 release. To install this patch, copy and overwrite files to the directory that NITRO-SDK 3.1 (05/11/2006 version) is installed.

* Apply this Plus Patch if you are using NITRO-SDK 3.1.

Caution:

If the NitroSDK library has been built before, an error may arise during the program build process. This is due to a cache data left over from the previous build.
If this happens, running "make clobber" at the NitroSDK root directory will delete this cache data.
Performing "make" at the same NitroSDK root directory at this point will remedy this problem.

Revision Content:

Contents revised in NitroSDK-3_1-patch-plus4-060807
    --------------------------------------------------------------------------
    
- (CARD) Fixed a bug where the system will not power down if a Game Card is removed while performing wireless communication.
    
There were certain cases where the system would not correctly power down if the Game Card was removed while performing wireless communication with the clamshell closed. This problem has been corrected. 
    
    
Issues Addressed in NitroSDK-3_1-patch-plus3-060802
    --------------------------------------------------------------------------
    
- (CARD) Fixed a bug where the power does not turn OFF when a Game Card is removed.
    
There was an issue where the power would not turn OFF, even if a Game Card was removed, when the DS was closed.
This problem has been corrected. 
    
    
Issues Addressed in NitroSDK-3_1-patch-plus2-060626
    --------------------------------------------------------------------------
    
- (PM) Fixed issue with the RTC alarm interrupt after recovering from sleep mode
    
There has been certain cases where the RTC alarm interrupt stopped working after recovering from sleep mode if the RTC alarm is not included in the recovery parameter. This problem was corrected.
    
    
Contents Revised in NitroSDK-3_1-patch-plus-060522
    --------------------------------------------------------------------------
    
- (PM) Fixed issue where the system will not power down if a Game Card is removed while the LCD is turned off.

As a result of the revisions described in "Note 36-13: (PM), Revisions to the PM_ForceToPowerOff and PM_ForceToPowerOffAsync functions," PM_ForceToPowerOffAsync could not be called while interrupts were disabled. As a result, the system did not power down when the card was removed with the LCD turned off, even if the clamshell was closed.
This has been fixed.
...
Source Files
	/NitroSDK/build/libraries/card/ARM9/src/card_pullOut.c
