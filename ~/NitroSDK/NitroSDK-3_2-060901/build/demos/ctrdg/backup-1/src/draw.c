...
static void OnVBlank(void)
{
    ...
    if ((p->draw_flag & DEMO_DRAW_FLIPPING) != 0)
    {
        ...
        /* Transmit the surface that was being drawn up until recently*/
        MI_CpuCopyFast(tmp_surface, dst_vram, sizeof(tmp_surface));
        /* Clear if an initial color has been designated*/
...
void InitDraw(void)
{
    ...
    if (!p->draw_flag)
    {
        ...
        /* shared render variable configuration*/
        ...
        /* VRAM-C direct display for dump displays, etc. in half-width characters*/
