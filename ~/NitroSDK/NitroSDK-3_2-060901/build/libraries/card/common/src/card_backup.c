...
/*---------------------------------------------------------------------------*
  ...
  Description:  Erases sectors and is main process for program requests.
                Called synchronously and asynchronously.
...
static void CARDi_RequestWriteSectorCommandCore(CARDiCommon * p)
{
    ...
    /* A failure will return if the process range is not an integral multiple in sector units.*/
...
/*---------------------------------------------------------------------------*
  ...
  $Log: card_backup.c,v $
  Revision 1.28  05/25/2006 02:49:51  yosizaki
  add CARDi_RequestWriteSectorCommand.

  Revision 1.27  05/01/2006 12:18:31  yosizaki
