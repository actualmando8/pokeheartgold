/*---------------------------------------------------------------------------*
  ...
  $Log: mb_wm_base.c,v $
  Revision 1.14  07/19/2006 04:41:30  yosizaki
  change default lifetime parameters.

  Revision 1.13  02/20/2006 02:35:20  seiki_masashi
  ...
  Revised so wait is inserted when using ichneumon
  ...
  Remove MBw function from mb_gameinfo.c, and cut out the scan parameter settings
  ...
  Fixed a problem where an error in WM_API return value was not returned with MB_CALLBACK_API_ERROR.
  ...
  In addition to Revision 1.44, revised to check NUL character also.

  Revision 1.44  10/08/2004 02:44:46  takano_makoto
  Revised to check nameLength of MPUserInfo with MP_Init.
  ...
  Support for changed specifications of StartMPEx. ignoreFatalErrorB

  Revision 1.40  09/28/2004 08:55:30  sato_masaki
  - Changed to issue OS_Panic when WM_STATECODE_FIFO_ERROR is generated in indication callback.
  ...
  Added a step to check whether initialization is complete in MB_Init.
  ...
  - Made changes so that the validity of the size of data for transmission is evaluated.
  ...
  Deleted the rand function and replaced it with the feature to make the beacon interval random using MAC address.
  ...
  Created MBi_Init() with tgid as an argument for backward compatibility.
  ...
  - The validity of work size for both parent and child is determined with SDK_STATIC_ASSERT.
  ...
  - Instead of separating the parent and child codes with SDK_SMALL_BUILD definitions, changed so that they can be build normally and still be separated.
  ...
  Dropped the tentative measures regarding the macro to calculate the size of send buffer and receive buffer and restored the original specification.
  ...
  Made changes so that MBi_CommEnd() is called if there is no scannable channel.
  ...
  - Changed WM_StartMP to Ex, and set it to Do Not Resend mode
  ...
  Instead of setting multiboot flag and GGID with MB_Init, set them with WM_SetGameInfo when sending beacon.
  ...
  Commented out the end communication process with "Beacon Indicate."
  ...
  Updated following the changes in WM_StartMP argument
  ...
  - Made changes so that callback is returned to MB_Comm layer even when the scan results in PARENT_NOT_FOUND.
...
/*
 * Determine if the requested size has the correct capacity.
 */
...
/*
 * Macro to calculate send and receiver buffer sizes between parent and child
...
#else
/* The fix that was used when the WM Library was unstable*/
/*
#define FRAME_LIFE_TIME 0xFFFF
*/
...
/*
 * WM system buffer.
 * Used to be a MBiParam member, but separated for MB_StartParentFromIdle() 
 */
static u8 *wmBuf;
 ...
 * Check if MP transmissions are possible.
   ...
   Added mpBusy flag to determination element. This is set when SetMP is run. It is used to prevent a further MP set between the time of SetMP and the callback return.
    ...
    case WM_APIID_SET_MP_DATA:
        {
            ...
            {
                MbWaitForWvr(13000);   // Make ARM9 wait for approximately 3 lines during IRQ interrupt
            }
            p_mbi_param->mpBusy = 0;
            if (buf->errcode == WM_ERRCODE_SUCCESS)
            {
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Obtains the value to add to the random beacon interval for the MAC address of each device.

  Arguments:    None.

  Returns:      u32 - Returns a random value in the range between 0 and 19 for each device.
 *---------------------------------------------------------------------------*/
...
int MB_Init(void *work, const MBUserInfo *user, u32 ggid, u32 tgid, u32 dma)
{
    ...
    else
    {
        ...
        /* Initialization of the lifetime and power-save mode setting value*/
...
static int MBi_StartParentCore(int channel)
{
    ...
    /* child max tranmission size (Byte)*/
...
int MBi_SendMP(const void *buf, int len, int pollbmp)
{
    ...
    {
    case MB_MODE_PARENT:
        errcode = MBi_SetMPData(p_mbi_param->callback_ptr, (u16 *)buf, size,    // * When MBi_ParentCallback is used, ParentCallback is linked even with mb_child. So, callback_ptr is used instead.
