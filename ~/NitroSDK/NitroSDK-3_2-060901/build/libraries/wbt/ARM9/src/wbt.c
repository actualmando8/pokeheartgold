/*---------------------------------------------------------------------------*
  ...
  $Log: wbt.c,v $
  Revision 1.27  08/01/2006 03:03:49  yosizaki
  fix about WBT_CMD_REQ_GET_BLOCK_DONE notification.

  Revision 1.26  06/13/2006 08:40:45  miya
  Dealt with data being registered during get_blockinfo polling

  Revision 1.25  03/01/2006 01:12:50  adachi_hiroaki
  Changed code so that warning is not displayed when compiled with gcc

  Revision 1.24  01/18/2006 02:12:38  kitase_hirotake
  do-indent

  Revision 1.23  12/16/2005 07:54:48  yosizaki
  fix result of user-callback.

  Revision 1.22  10/20/2005 04:01:20  yosizaki
  fix system-callback on GetBlockInfo-done.

  Revision 1.21  02/28/2005 05:26:32  yosizaki
  do-indent.

  Revision 1.20  02/18/2005 12:03:23  yosizaki
  fix around hidden warnings.

  Revision 1.19  10/15/2004 07:13:21  miya
  Dealt with get_blockinfo polling

  Revision 1.18  10/15/2004 05:18:58  miya
  Fixed bugs around get_blockinfo

  Revision 1.17  10/04/2004 08:38:42  miya
  modified comments

  Revision 1.16  09/21/2004 05:29:21  yosizaki
  adjust some variable types.

  Revision 1.15  09/14/2004 01:33:57  miya
  Added WBT_SetPacketSize function.

  Revision 1.14  09/09/2004 06:41:14  miya
  small fix.

  Revision 1.13  09/04/2004 08:57:22  miya
  Changed the WBT_CMD_PREPARE_SEND_DATA event processing

  Revision 1.12  09/03/2004 12:03:09  miya
  Fixed the bug during WBT_CMD_PREPARE_SEND_DATA event processing

  Revision 1.11  09/03/2004 09:29:31  seiki_masashi
  small fix

  Revision 1.10  09/01/2004 03:59:50  miya
  Deleted PutBlock, RequestRegisterBlock, ResponseRegisterBlock, and Done commands.

  Revision 1.9  08/28/2004 11:26:42  miya
  Added WBT_PutUserData function

  Revision 1.8  08/27/2004 07:07:52  miya
  Made changes to names, etc.

  Revision 1.7  08/26/2004 12:59:23  miya
  Made changes to names, etc.

  Revision 1.6  08/26/2004 05:00:53  miya
  bug fixes

  Revision 1.5  08/26/2004 03:19:00  miya
  Added the WBT_RegisterUserDefinedBlock function

  Revision 1.4  08/25/2004 12:28:48  miya
  Added RegBlock-related commands

  Revision 1.3  08/24/2004 05:46:37  miya
  comments added
...
#pragma unused(size)
    ...
    if (recv_token->command_state == COMMAND_STATE_GET_REQUEST_FROM_PEER)
    {
	    ...
	    cmd_param->peer_param[aid].is_sending_null_blockinfo = FALSE;
        }
    }
    return ret_size;
}
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Checks whether GetBlockInfo request has come from other station.
                If it has come, makes response command.
                However, if an unregistered BlockInfo is requested,
                no response command is created.
...
#pragma unused(size)
  ...
  /****** Request from peer (Respond immediately) **************/
    if (recv_token->command_state == COMMAND_STATE_GET_REQUEST_FROM_PEER)
    {
        ...
        {
	    if( cmd_param->peer_param[aid].is_sending_null_blockinfo == TRUE )
	    {
	      data_ptr = NULL;
	    }
	    else
	    {
	      ...
	      //  OS_TPrintf("is_sending_null_blockinfo -> FALSE \n");
	    }
        }
    }
    return ret_size;
...
/*---------------------------------------------------------------------------*
  ...
  Description:  Checks whether GetBlockInfo request has come from other station.
                If it has come, makes response command.
                However, if an unregistered BlockInfo is requested,
                a response command with a 0 embedded is created.
  ...
  /****** Request from peer (Respond immediately) **************/
    if (recv_token->command_state == COMMAND_STATE_GET_REQUEST_FROM_PEER)
    {
	    ...
	    else
	    {
	      ...
	      if( data_ptr == NULL )
	      {
	        cmd_param->peer_param[aid].is_sending_null_blockinfo = TRUE;
	      }
	    }
