/*---------------------------------------------------------------------------*
  ...
  Revision 1.15  07/27/2006 01:50:25  okubata_ryoma
  small fix

  Revision 1.14  07/25/2006 04:37:58  seiki_masashi
  Revised the CP used section to no interrupt
  ...
  Reorganized various parameters
...
/*---------------------------------------------------------------------------*
  ...
  Arguments:    resultEntries   Pointer to array that enters pointer to recognition result.
                                If only less than required number are recognized,
...
// Further up you go, the faster it will be. Precision does not drop that much.
...
#define angleScores_(x,y) (*(angleScores + (x) * (maxPointCount+1) + (y)))
    ...
    *score = 0;
    ...
    // Lines that are too different will be treated as zero similarity, and not calculated any further.
    ...
    // Early calculation regarding scoring of angles in angleScores[][].
    ...
    // Similarity calculation phase
    {
        ...
        for (iInput = 0; iInput < inputSize; iInput++)
        {
            ...
            if (pair >= 0)
            {
                // Whether I am the best pair for the target
...
//#define PRC_DEBUG_RECOGNIZE_DEEPLY
        ...
        for (iMatch = 0; iMatch < nMatches - 1; iMatch++)
        {
            ...
            {
                // [0, 0] [0, 0] and corresponding point may overlap just at the beginning.
                continue;
            }
